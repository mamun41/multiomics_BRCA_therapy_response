...
def optimise_logres_featsel(X, y, cut, cv, label='Response', prefix='someresponse', metric='roc_auc_ovo'):
    # Pipeline components
    ...
    #pipe = Pipeline(steps=[('dropcoll', dropcoll), ('scaler', scaler), ('logres', logres)])
    # Parameter ranges
    param_grid = { 'kbest__k': np.arange(2,X.shape[1],1),
                    'logres__C': np.logspace(-3,3,30),
                    'logres__l1_ratio': np.arange(0.1,1.1,0.1) }
    # Optimisation
    search = RandomizedSearchCV(pipe, param_grid, cv=cv, scoring=metric, return_train_score=True, n_jobs=-1, verbose=0, n_iter=1000, random_state=0)
...
def optimise_SVC_featsel(X, y, cut, cv=5, label='Response', prefix='someresponse'):
    ...
    #pipe = Pipeline(steps=[('dropcoll', dropcoll), ('scaler', scaler), ('svc', svc)])
    ...
    # Optimisation
    search = RandomizedSearchCV(pipe, param_grid, cv=cv, scoring='roc_auc_ovo',return_train_score=True, n_jobs=-1, verbose=0, n_iter=1000, random_state=0)
...
def optimise_rf_featsel(X, y, cut, cv=5, label='Response', prefix='someresponse'):
    ...
    #pipe = Pipeline(steps=[('dropcoll', dropcoll), ('scaler', scaler), ('rf', rf)])
    # Parameter ranges
    ...
    # Optimisation
    search = RandomizedSearchCV(pipe, param_grid, cv=cv, scoring='roc_auc_ovo',return_train_score=True, n_jobs=-1, verbose=0,n_iter=1000, random_state=1)
...
def plot_and_refit(X, y, model, cv, label='Response', prefix='someresponse',feats='features',ids=None):
    ...
    fout = open('../submissions/'+'patpreds_cv_'+feats+'.txt', 'a')
    cv_models = []
    for i,(tr,ts) in enumerate(cv):
        model.fit(X.iloc[tr, :], y.iloc[tr])
        cv_models.append(deepcopy(model))

        #y_pred = model.predict_proba(X.iloc[ts, :])
        ...
        roc_auc = roc_auc_score(ytest, y_pred, multi_class='ovo')
        aucs.append(roc_auc)
    ...
    fpr, tpr, thresholds = roc_curve(yreals, ypreds)
    tprs.append(interp(mean_fpr, fpr, tpr))
    tprs[-1][0] = 0.0
    ##plt.plot(fpr, tpr, lw=1, alpha=0.3,
    # label='ROC fold %d (AUC = %0.2f)' % (i+1, roc_auc))

    np.save('../submissions/' +feats +'_ypreds_cv.npy',ypreds )
    np.save('../submissions/' +feats +'_yreals_cv.npy', yreals)
    np.save('../submissions/' +feats + '_aucs.npy', aucs)
    ...
    #plt.show()
    #plt.savefig('../submissions/' + feats + '_roc.png', format='png', bbox_inches='tight')
    ...
    #plt.xlabel('FPR')
    #plt.ylabel('TPR')
    #plt.savefig('../submissions/'+feats+'_auc.png', format='png', bbox_inches='tight')
    #plt.close()
    #plt.show()

    #f = open('/data/output.txt', 'a')
    f = open('../submissions/'+'output_'+feats+'.txt', 'a')
...
def run_all_models(X,y,splits,cut):
    logres_result_auc = optimise_logres_featsel(X,y,cut=cut,cv=splits,metric='roc_auc_ovo')
    ...
    #averaged_models = AveragingModels(models=(logres_result_auc, svc_result))
    results = {}
...
def refit_all_models2(X,y,results,splits,whichFeats,criterion,patID=None):
    refit = {}
    prefix = criterion
    feats = whichFeats
    ids = patID
    for model in results.keys():
        ...
        for i, (tr, ts) in enumerate(splits):
            try:
                mb=results[model].best_estimator_
            except:
                mb = results[model]
            mb.fit(X.iloc[tr, :], y.iloc[tr])
            cv_models.append(deepcopy(mb))

            y_pred = mb.predict_proba(X.iloc[ts, :])
                # y_pred = model.predict_proba(X.iloc[ts,:])[:,1]
