def defineFeatures(whichFeat, her2=0):
    import pickle
    if her2==0:
        fam = pickle.load(open('../inputs/my_list427.pkl', 'rb'))
    ...
    if 'LOO' in whichFeat:
        ...
        feats = fam['mutation']+fam['others']+fam['DEG']+fam['immune_proliferation']
    elif whichFeat == 'diablo':
        feats = fam['mutation']+fam['DEG']+fam['immune_proliferation']
    elif whichFeat == 'all':
        feats = fam['others']+fam['mutation']+fam['DEG']+fam['immune_cell']+fam['molecular_markers']
    elif whichFeat == 'diablo427':
        feats = fam['mutation']+fam['DEG']+fam['immune_cell']+fam['molecular_markers']
    elif whichFeat == 'mutation':
        feats = fam['mutation']
    elif whichFeat == 'DEG':
        feats = fam['DEG']
    elif whichFeat == 'immune_cell':
        feats = fam['immune_cell']
    elif whichFeat == 'molecular_markers':
        feats = fam['molecular_markers']
    elif whichFeat == 'common':
        feats = fam['common']
    elif whichFeat == 'allpositive':
        feats = fam['allpositive']
    elif whichFeat == 'DEG_cell':
        feats = fam['DEG']+fam['immune_cell']
    return feats
...
def defineTrainingSets(df_train, feats, her2=0):
    ...
    df_train_reshuffled = df_train.copy().sample(frac=1, random_state=0).reset_index(drop=True) #rows randomly shuffled
...
## SETUP
    from classification_models import optimise_logres_featsel, optimise_SVC_featsel, optimise_rf_featsel, run_all_models, refit_all_models, refit_all_models2#, final_test, test_all_models
    import pandas as pd
    import pickle
    plotStyle()
...
## OUTPUT
    f = open('../submissions/'+'output_'+whichFeats+'.txt', 'a')
    f.write('-----'+whichFeats+'------\n')
    f.close()

## INPUT
    her2 = int(her2_str)
    df_train = pd.read_csv('../inputs/all.csv')
...
## MODELS
    ...
    #pcr_models = run_all_models(Xtrain, ytrainCateg, splits, float(rcut))
    pcr_models = run_all_models(Xtrain, ytrain_pCR, splits, float(rcut))

    #pcr_refits = refit_all_models2(Xtrain, ytrainCateg, pcr_models, splits, whichFeats, 'pCR', ytrainID)
    pcr_refits = refit_all_models(Xtrain, ytrain_pCR, pcr_models, splits, whichFeats, 'pCR', ytrainID)
    with open('../submissions/'+whichFeats+'_pcr_2_models_nkb.pkl', 'wb') as f:
        pickle.dump(pcr_models, f)
    with open('../submissions/'+whichFeats+'_pcr_2_refits_nkb.pkl', 'wb') as f:
