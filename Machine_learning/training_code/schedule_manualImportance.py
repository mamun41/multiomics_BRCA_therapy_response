...
singularity exec -H /the/directory/with/the/files:/home -B /the/output/directory/results_{stamp}_r{rcut}_her2{her2word}_rs{rs}:/data pythonenv.img python run_models.py {feats} {her2num} {rcut} {rs}
...
def main():
    ...
    for i,combi in enumerate(parameter_combinations):
        ...
        for feat_i in all_feats:
            ...
            if not os.path.exists('/the/output/directory/{}'.format(outdir)):
                os.makedirs('/the/output/directory/{}'.format(outdir))
                print('Making: /the/output/directory/{}'.format(outdir))
